...
std::pair<std::vector<float>, std::vector<float>> mixing_infer(std::string model_bin_path, std::string input0_path, std::string input1_path)
{
    ...
    if(DEBUG){
        std::cout << "OUT Data in vir_addr:" << std::endl;
        for (size_t i = 0; i < 100; ++i)
        {
    ...
    // std::cout << "prepare return data" << std::endl;
    std::vector<float> vec1(out_vir_addr_float, out_vir_addr_float + 1024); // * sizeof(float)); 不能加，BUG所在
    std::vector<float> vec2(out_vir_addr_float2, out_vir_addr_float2 + 5 * 1024); // * sizeof(float));
    // std::cout << "finish return data" << std::endl;

    // std::cout << "ready to release task handle" << std::endl;
    hbDNNReleaseTask(task_handle);
    // std::cout << "finish release task handle" << std::endl;
    for(int i=0; i<input_count; i++){
...
std::vector<float> head_infer(std::string model_bin_path, std::string input0_path)
{
    hbPackedDNNHandle_t packed_dnn_handle;
    hbDNNHandle_t dnn_handle;
    char const *modelFileNames[] = {
